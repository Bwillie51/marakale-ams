...
export default function PublicHotelMarketingWebsite() {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between bg-slate-50 text-slate-800 antialiased font-sans">
      ...
      {/* 3.1 UPCOMING SEASONAL EVENTS PROMOTIONS */}
      {/* I will delete this part later, but I will keep it for now for future reference */}
      {/* This events will be added through the Sanity CMS in the future for dynamic updates */}
      <section>
        <div className="max-w-7xl mx-auto px-6 md:px-12 py-6">
          <div className="border-b border-slate-200 pb-2">
            <h3 className="text-xs font-black uppercase text-slate-400 tracking-wider flex items-center gap-1.5">
              <Calendar className="w-4 h-4 text-sky-500" /> Upcoming Events
            </h3>
          </div>
        </div>
      </section>
      <section className="bg-slate-50 py-12 px-6 md:px-12">
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white border border-slate-200 p-6 rounded-xl shadow-sm flex flex-col justify-between space-y-4">
              <div className="space-y-2">
                {/* Event Badge - Dynamic from Sanity CMS */}
                <span className="text-[9px] uppercase font-black tracking-widest bg-sky-50 text-sky-700 border border-sky-200 px-2 py-0.5 rounded">
                  {"X-mas Eve"}
                </span>
                <h4 className="text-base font-black text-slate-900">{"Christmas Celebration"}</h4>
                <p className="text-xs text-slate-500 leading-relaxed font-normal">{"Join us for an unforgettable evening of live music, gourmet dining, and exclusive rooftop cocktails as we celebrate the longest day of the year."}</p>
              </div>
              <Link href="/login" className="text-slate-900 font-bold text-xs uppercase tracking-wider flex items-center gap-1 hover:text-slate-700">
                {"Reserve Your Spot"} <ArrowRight className="w-3.5 h-3.5" />
              </Link>
            </div>
            <div className="bg-white border border-slate-200 p-6 rounded-xl shadow-sm flex flex-col justify-between space-y-4">
              <div className="space-y-2">
                ...
                <h4 className="text-base font-black text-slate-900">{"5th Anniversary Celebration"}</h4>
                <p className="text-xs text-slate-500 leading-relaxed font-normal">{"Celebrate the bounty of the season with artisanal food stalls, craft workshops, and live entertainment in our beautifully decorated courtyard."}</p>
              </div>
              <Link href="/login" className="text-slate-900 font-bold text-xs uppercase tracking-wider flex items-center gap-1 hover:text-slate-700">
                {"Join the Celebration"} <ArrowRight className="w-3.5 h-3.5" />
              </Link>
            </div>
          </div>
        </div>
      </section>
      {/* Photos and videos of prviouse events will be added here in the future */}
      <section className="bg-slate-50 py-12 px-6 md:px-12">
        <div className ="max-w-7xl mx-auto space-y-6">
          <div className="border-b border-slate-200 pb-2">
            <h3 className="text-xs font-black uppercase text-slate-400 tracking-wider flex items-center gap-1.5">
              <Sparkles className="w-4 h-4 text-amber-500" /> Past Event Highlights
            </h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* Placeholder for past event media */}
            <div className="bg-white border border-slate-200 p-6 rounded-xl shadow-sm flex flex-col justify-between space-y-4">
              ...
              <h4 className="text-base font-black text-slate-900">{"Past Event Title"}</h4>
              <p className="text-xs text-slate-500 leading-relaxed font-normal">{"Brief description of the past event, highlighting key moments and experiences."}</p>
            </div>
            {/* Repeat for other past events */}
          </div>
        </div>
      </section>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-xs font-black text-slate-900">
                      From <span className="text-sm font-black text-emerald-600">K{tier.priceFrom}</span> <span className="text-[10px] text-slate-400 font-normal">/ night</span>
                    </span>
                  </div>
